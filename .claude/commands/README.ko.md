...
| 커맨드 | 목적 | 주요 기능 |
|---------|---------|-------------|
| `do.md` | 직접 실행 | 플래너 없음, /do-sonnet으로 모델 재정의 |
...
| `load-context.md` | 컨텍스트 로드 | Read tool, 토큰 경제 |
...
| 명령어 | 사용 시점 | 리소스 사용량 |
|---------|-------------|--------------|
| `/do` | 명확히 정의된 간단한 작업 (< 3개 파일) | 최소 (Haiku 기본) |
...
| `/dplan` | 연구가 많은 복잡한 아키텍처 | 높음 (Sonnet + MCP 도구) |

### 품질 보증
| 명령어 | 사용 시점 | 리소스 사용량 |
|---------|-------------|--------------|
| `/review` | 구현 후 품질 체크 | 최소 (Haiku, 읽기 전용) |
...
/review src/auth/
```
**Quota:** 중간 (Sonnet 계획 + Haiku 구현 + Haiku 검토)
...
```
**Quota:** 높음 (Sonnet + Sequential Thinking + Perplexity + Context7)
...
|--------|-----|------------|----------|-------|--------|
| **모델** | Haiku (기본) | Sonnet | Opus | Sonnet → Haiku | Sonnet + MCP |
| **비용** | 기본 | 중간 | 높음 | 중간 | 높음 |
| **계획** | 없음 | 없음 | 없음 | 아키텍처 설계 | 심층 연구 |
